...
static std::string HttpGet(std::string url)
{
    ...
    if (http_fetch((const char *)url.c_str(), &buf) == -1) {
        ...
        WEATHER_LOG_WARNING("URL[%s] http request error!", url.c_str());
        std::string errStr = http_strerror();
        if (errStr.compare("Success") != 0) {
            http_perror( const_cast<char *>(errStr.c_str()));
            cout << errStr << endl;
            WEATHER_LOG_WARNING("URL[%s] error code[%s].", url.c_str(), errStr.c_str());
        }
    ...
    WEATHER_LOG("Http Got Info [%s] End.", content.c_str());
...
WeatherForecast:: WeatherForecast(std::string cityCodeURL)
{
    ...
    WEATHER_LOG("WeatherForecast::CityCode code: [%s].", cityCode.c_str());
}
...
void WeatherForecast:: SetURL(std::string urlParten)
{
    ...
    WEATHER_LOG("infoURL: [%s].", infoURL.c_str());
    ...
    url.append(cityCode);
    url.append(urlPart);
    WEATHER_LOG("The WeatherForecast main url: [%s].", url.c_str());
    ...
    WEATHER_LOG("The WeatherForecast back url: [%s].", urlBack.c_str());
...
std::string WeatherForecast:: GetInfo()
{
    ...
    if (content.compare("ERROR_HTTP_REQUEST") == 0 || content.find("<html>") != string::npos) {
        ...
        if (content.compare("ERROR_HTTP_REQUEST") == 0 || content.find("<html>") != string::npos) { //weather info get error
            WEATHER_LOG_WARNING("Weather Forecast url[%s] http request error.", urlBack.c_str());
            isErrorStatus = isErrorStatus | (1 << 1); // 2 = "0010"
            return "ERROR_REQUEST";
        }
    }
    WEATHER_LOG("The WeatherForecast Http Get Info content: [%s]", content.c_str());
    ...
    WEATHER_LOG("The WeatherForecast GetInfo : [%s] end.", retValue.c_str());
    return retValue;
}
...
AQIQuery:: AQIQuery(std::string cityCodeURL)
{
    ...
    WEATHER_LOG("The AQIQuery cityCode is: [%s] end.", cityCode.c_str());
}
...
void AQIQuery:: SetURL(std::string urlParten)
{
    ...
    WEATHER_LOG("The AQIQuery queryURL is: [%s].", queryURL.c_str());
    ...
    url.append(cityCode);
    url.append(urlPart);
    WEATHER_LOG("The AQIQuery main url is: [%s].", url.c_str());
    ...
    urlBack.append(cityCode);
    urlBack.append(urlPart);
    WEATHER_LOG("The AQIQuery back url is: [%s].", urlBack.c_str());
...
std::string AQIQuery:: GetInfo()
{
    ...
    if (content.compare("ERROR_HTTP_REQUEST") == 0 || content.find("<html>") != string::npos) {
        WEATHER_LOG_WARNING("The AQIQuery use urlBack[%s].", urlBack.c_str());
        content = HttpGet(urlBack);
        if (content.compare("ERROR_HTTP_REQUEST") == 0 || content.find("<html>") != string::npos) {  //aqi info get error
            WEATHER_LOG_WARNING("The AQIQuery back url[%s] http request error.", urlBack.c_str());
            isErrorStatus = isErrorStatus | (1 << 2); // 4 = "0100"
            return "ERROR_REQUEST";
        }
    }
    cout << content << endl;
    WEATHER_LOG("The AQIQuery Get Info content[%s].", content.c_str());
    ...
    WEATHER_LOG("The AQIQuery GetInfo retValue: [%s] end.", retValue.c_str());
    return retValue;
}
...
int Weather:: GetWeatherInfo(std::string weatherUrlParten, std::string aqiUrlParten)
{
    ...
    WEATHER_LOG("The WeatherForecast URL Parten[%s], The AQIQuery URL Parten[%s].", weatherUrlParten.c_str(), aqiUrlParten.c_str());
    WeatherForecast:: SetURL( weatherUrlParten);
    AQIQuery:: SetURL( aqiUrlParten);
    WEATHER_LOG("The WeatherForecast request URL[%s], The AQIQuery query URL[%s].", WeatherForecast::GetURL().c_str(), AQIQuery::GetURL().c_str());
    ...
    WEATHER_LOG("GetWeatherInfo: [%s], isErrorStatus[%d] end.", content.c_str(), isErrorStatus);
